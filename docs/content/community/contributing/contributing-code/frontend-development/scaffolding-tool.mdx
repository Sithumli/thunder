...
<ProductName /> Create (`@thunder/create`) is a CLI scaffolding tool that automates the creation of feature packages and shared packages in the <ProductName /> frontend workspace. The tool generates complete, production-ready package structures with pre-configured build tools, testing infrastructure, and best practices.
